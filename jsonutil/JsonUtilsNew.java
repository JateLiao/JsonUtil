...
public class JsonUtilsNew {
            ...
            if (isBasicType(objClass)) { // 四类八种基本类型，以及String
                ...
                for (Field field : fields) {
                    ...
                    if (isBasicType(field.getType())) { // 四类八种基本类型，以及String
                        ...
                        consrtuctMapJsonValue(obj, field, sb, false);
                    }
                ...
                for (Map.Entry<Object, Object> entry : mapObj.entrySet()) {
                    ...
                    if (isBasicType(entry.getValue().getClass())) { // 四类八种基本类型，以及String
                        constructJsonValue(entry.getValue(), sb);
                    } else if (isListType(entry.getValue().getClass())) { // List类型
                        constructJsonValue(entry.getValue(), sb);
                    } else if (isDefinedModel(entry.getValue().getClass())) { // 其他自定义类型
                        constructJsonValue(entry.getValue(), sb);
                    } else if (isMapType(entry.getValue().getClass())) {
                        constructJsonValue(entry.getValue(), sb);
                    }
                }
    ...
    private static void constructModelJsonValue(Object obj, Field field, StringBuffer sb, boolean isFromList) throws Exception {
        ...
        for (Field fd : fds) {
            ...
            } else if (isMapType(fd.getType())) {
                consrtuctMapJsonValue((isFromList ? obj : field.get(obj)), fd, sb, false);
            }
    ...
    private static void constructListJsonValue(Object obj, Field field, StringBuffer sb, boolean isFromList) throws Exception {
        ...
        for (Object object : list) {
            ...
            if (isBasicType(clazz)) {
                ...
                consrtuctMapJsonValue(object, null, sb, true);
            }
        }
     ...
     * Map转化.
     */
    private static void consrtuctMapJsonValue(Object obj, Field field, StringBuffer sb, boolean isFromList) throws Exception {
        sb.append("\"").append(field.getName()).append("\":");
        Map<Object, Object> mapObj = (Map<Object, Object>) field.get(obj);
        constructJsonValue(mapObj, sb);
    }
